...
typedef struct {
	...
	unsigned int dbwSystemError : 1; // bit 22
	unsigned int isTriggerError : 1; // bit 23
	unsigned int hasFatalError : 1; // bit 24
	unsigned int isWarnNow : 1; // bit 25
	unsigned int launchTriggered : 1; // bit 26
	unsigned int rpmHardCut : 1; // bit 27
	...
	scaled_percent boostDuty; //128
		...
		float tpsAdcMin; //132
		float tpsAdcMax; //136
		scaled_voltage tpsVoltage; //140
		scaled_voltage mapVoltage; //142 size
		scaled_voltage pedalVoltage; //144
		scaled_percent gpPwm1Duty;
		scaled_percent heaterDuty;
		int16_t wideBandState;
		int16_t wideBandError;
		int16_t wideBandIdent;   // 154
		scaled_voltage widebandUa;
		scaled_voltage widebandUr;
		scaled_voltage	cltVoltage; //160
		scaled_voltage iatVoltage;
