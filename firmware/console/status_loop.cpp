...
void updateTunerStudioState(TunerStudioOutputChannels *tsOutputChannels DECLARE_ENGINE_PARAMETER_SUFFIX) {
	...
	tsOutputChannels->mapVoltage = getVoltageDivided("map", engineConfiguration->map.sensor.hwChannel PASS_ENGINE_PARAMETER_SUFFIX);



	tsOutputChannels->tpsVoltage = engine->engineState.tpsVoltageMCU * CONFIG(analogInputDividerCoefficient);
	...
	tsOutputChannels->rpmHardCut = engine->rpmHardCut;
