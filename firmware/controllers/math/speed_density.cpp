...
void setDefaultVETable(DECLARE_ENGINE_PARAMETER_SIGNATURE) {
	setRpmTableBin(config->fuelRpmBins, FUEL_RPM_COUNT);
	setRpmTableBin(config->fuel2RpmBins, FUEL_RPM_COUNT);
	fuelMap.setAll(80);
	fuel2Map.setAll(80);
...
void initSpeedDensity(DECLARE_ENGINE_PARAMETER_SIGNATURE) {
	...
	fuel2Map.init(config->fuel2Table, config->fuel2LoadBins, config->fuel2RpmBins);
