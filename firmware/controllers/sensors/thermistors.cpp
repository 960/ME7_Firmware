...
temperature_t getTemperatureC(ThermistorConf *cfg, ThermistorMath *tm, bool useLinear DECLARE_ENGINE_PARAMETER_SUFFIX) {
	...
	float cltVolt = getVoltageDivided("clt", engineConfiguration->clt.adcChannel PASS_ENGINE_PARAMETER_SUFFIX);
	float iatVolt = getVoltageDivided("iat", engineConfiguration->iat.adcChannel PASS_ENGINE_PARAMETER_SUFFIX);



#if EFI_TUNER_STUDIO

	tsOutputChannels.cltVoltage = cltVolt;
	tsOutputChannels.iatVoltage = iatVolt;
