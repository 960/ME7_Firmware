...
//	prin("Finished %d iterations of 'setPinValue()' in %dms\r\n", count, time);
	...
	if (result != 0) {
		...
		scheduleMsg("Finished %d iterations of 'chTimeNow()' in %dms", count, time);
	}
	...
	if (result != 0) {
		...
		scheduleMsg("Finished %d iterations of 'chTimeNow()' with chSysLock in %dms", count, time);
	}
	...
	time = currentTimeMillis() - start;
	if (result != 0)
		scheduleMsg("Finished %d iterations of 'currentTimeMillis' in %dms", count, time);
}
...
static void testRusefiMethods(const int count) {
	...
	time = currentTimeMillis() - start;
	if (tempi != 0)
		scheduleMsg("Finished %d iterations of getBaseFuel in %dms", count, time);
...
//	time = currentTimeMillis() - start;
//	if (tempi != 0)
//		scheduleMsg("Finished %d iterations of getFuelMs in %dms", count, time);
	...
	time = currentTimeMillis() - start;
	if (tempi != 0)
		scheduleMsg("Finished %d iterations of updateSlowSensors in %dms", count, time);
}
...
static void testMath(const int count) {
	...
	time = currentTimeMillis() - start;
	if (temp64 != 0) {
		scheduleMsg("Finished %d iterations of int64_t summation in %dms", count, time);
	}
	...
	time = currentTimeMillis() - start;
	if (temp64 == 0) {
		scheduleMsg("Finished %d iterations of int64_t multiplication in %dms", count, time);
	}
	...
	for (int i = 0; i < count; i++)
		;
	time = currentTimeMillis() - start;
	scheduleMsg("Finished %d iterations of empty loop in %dms", count, time);
	...
	if (tempi == 0) {
		...
		scheduleMsg("Finished %d iterations of uint32_t summation in %dms", count, time);
	}
	...
	if (tempi != 0) {
		...
		scheduleMsg("Finished %d iterations of uint32_t division in %dms", count, time);
	}
	...
	if (temp64 != 0) {
		...
		scheduleMsg("Finished %d iterations of int64_t division in %dms", count, time);
	}
	...
	time = currentTimeMillis() - start;
	if (tempf != 0) {
		scheduleMsg("Finished %d iterations of float summation in %dms", count, time);
	}
	...
	if (tempf != 0) {
		...
		scheduleMsg("Finished %d iterations of float multiplication in %dms", count, time);
	}
	...
	if (tempf != 0) {
		...
		scheduleMsg("Finished %d iterations of float division in %dms", count, time);
	}
	...
	if (tempf != 0) {
		...
		scheduleMsg("Finished %d iterations of float log in %dms", count, time);
	}
	...
	if (tempd != 0) {
		...
		scheduleMsg("Finished %d iterations of double summation in %dms", count, time);
	}
	...
	if (tempd != 0) {
		...
		scheduleMsg("Finished %d iterations of double division in %dms", count, time);
	}
	...
	if (tempd != 0) {
		...
		scheduleMsg("Finished %d iterations of double log in %dms", count, time);
	}
}

static void runTests(const int count) {
	scheduleMsg("Running tests: %d", count);
...
static void timeInfo(void) {
	scheduleMsg("chTimeNow as seconds = %d", getTimeNowSeconds());
	scheduleMsg("hal seconds = %d", halTime.get() / (long)CORE_CLOCK);

#if EFI_RTC
	int unix = rtcGetTimeUnixSec(&RTCD1) - rtcStartTime;
	scheduleMsg("unix seconds = %d", unix);
#endif
}
