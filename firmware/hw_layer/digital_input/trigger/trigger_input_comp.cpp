...
static void comp_shaft_callback(COMPDriver *comp) {
	...
	if (status & COMP_IRQ_RISING) {
		signal = isPrimary ? (engineConfiguration->trigger1Edge ? SHAFT_PRIMARY_FALLING : SHAFT_PRIMARY_RISING) : 
			(engineConfiguration->trigger2Edge ? SHAFT_SECONDARY_FALLING : SHAFT_SECONDARY_RISING);
	...
	if (status & COMP_IRQ_FALLING) {
		signal = isPrimary ? (engineConfiguration->trigger1Edge ? SHAFT_PRIMARY_RISING : SHAFT_PRIMARY_FALLING) : 
			(engineConfiguration->trigger2Edge ? SHAFT_SECONDARY_RISING : SHAFT_SECONDARY_FALLING);
		hwHandleShaftSignal(signal, stamp);
...
#ifdef EFI_TRIGGER_COMP_ADAPTIVE_HYSTERESIS
	scheduleMsg("startTIPins(): ADAPTIVE_HYSTERESIS enabled!");
#endif /* EFI_TRIGGER_COMP_ADAPTIVE_HYSTERESIS */
...
void stoppinTrigger(void) {
	...
	for (int i = 0; i < TRIGGER_SUPPORTED_CHANNELS; i++) {
		if (isConfigurationChanged(bc.pinTrigger[i])) {
			turnOffTriggerInputPin(activeConfiguration.bc.pinTrigger[i]);
		}
	}
