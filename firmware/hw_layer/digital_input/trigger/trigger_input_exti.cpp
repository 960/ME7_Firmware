...
static void shaft_callback(void *arg) {
	...
	if (rise) {
		signal = isPrimary ?
					(engineConfiguration->trigger1Edge ? SHAFT_PRIMARY_FALLING : SHAFT_PRIMARY_RISING) :
					(engineConfiguration->trigger2Edge ? SHAFT_SECONDARY_FALLING : SHAFT_SECONDARY_RISING);
	} else {
		signal = isPrimary ?
					(engineConfiguration->trigger1Edge ? SHAFT_PRIMARY_RISING : SHAFT_PRIMARY_FALLING) :
