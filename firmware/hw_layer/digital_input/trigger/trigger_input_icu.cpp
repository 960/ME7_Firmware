...
// todo: support for 3rd trigger input channel
	...
	trigger_event_e signal = isPrimary ? (engineConfiguration->trigger1Edge ? SHAFT_PRIMARY_FALLING : SHAFT_PRIMARY_RISING) : (engineConfiguration->trigger2Edge ? SHAFT_SECONDARY_FALLING : SHAFT_SECONDARY_RISING);
...
static void shaftFallingCallback(bool isPrimary) {
	...
	trigger_event_e signal =
			isPrimary ? (engineConfiguration->trigger1Edge ? SHAFT_PRIMARY_RISING : SHAFT_PRIMARY_FALLING) : (engineConfiguration->trigger2Edge ? SHAFT_SECONDARY_RISING : SHAFT_SECONDARY_FALLING);
	hwHandleShaftSignal(signal, stamp);
}
