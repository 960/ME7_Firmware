...
void enableDefaultCan(DECLARE_ENGINE_PARAMETER_SIGNATURE) {
...
void initCan(void) {
	...
	if (!isValidCanTxPin(CONFIG_OVERRIDE(canTxPin))) {
		warning(CUSTOM_OBD_70, "invalid CAN TX %s", hwPortname(CONFIG_OVERRIDE(canTxPin)));
		return;
	}

	if (!isValidCanRxPin(CONFIG_OVERRIDE(canRxPin))) {
		warning(CUSTOM_OBD_70, "invalid CAN RX %s", hwPortname(CONFIG_OVERRIDE(canRxPin)));
		return;
	}
