...
#define MODE_SEMIAUTO 0x01
#define MODE_MANUAL 0x02
#define MODE_HALL 0x03

//VRSCONFIG0
#define PEAK_TIME_10_US  0x00
#define PEAK_TIME_250_US  0x01

#define VR_FILTER_1_US  0x00
#define VR_FILTER_5_US  0x01
#define VR_FILTER_10_US 0x02
#define VR_FILTER_20_US 0x03

#define VR_VOLTAGE_50_MV  0x00
#define VR_VOLTAGE_150_MV 0x01
#define VR_VOLTAGE_350_MV 0x02
#define VR_VOLTAGE_550_MV 0x03
/* C0 */
#define CMD_READ			(0 << 0)
...
int tle8888SpiStartupExchange(struct tle8888_priv *chip) {
	...
	if (CONFIG(diagnoseTLE8888)) {
		tle8888_dump_regs();
	}


	uint8_t vrsconfig0 = 0;
	if (CONFIG(tle8888VrPeakTime) == T_10_US) {
		vrsconfig0 |=  PEAK_TIME_10_US << 0;
	} else if (CONFIG(tle8888VrPeakTime) == T_250_US) {
		vrsconfig0 |=  PEAK_TIME_250_US << 0;
	}

	if (CONFIG(tle8888VrFilter) == F_1_US) {
		vrsconfig0 |=  VR_FILTER_1_US << 1;
	} else if (CONFIG(tle8888VrFilter) == F_5_US) {
		vrsconfig0 |=  VR_FILTER_5_US << 1;
	} else if (CONFIG(tle8888VrFilter) == F_10_US) {
		vrsconfig0 |=  VR_FILTER_10_US << 1;
	} else if (CONFIG(tle8888VrFilter) == F_20_US) {
		vrsconfig0 |=  VR_FILTER_20_US << 1;
	}

	if (CONFIG(tle8888VrDetectionVoltage) == V_50_MV) {
		vrsconfig0 |=  VR_VOLTAGE_50_MV << 6;
	} else if (CONFIG(tle8888VrDetectionVoltage) == V_150_MV) {
		vrsconfig0 |=  VR_VOLTAGE_150_MV << 6;
	} else if (CONFIG(tle8888VrDetectionVoltage) == V_350_MV) {
		vrsconfig0 |=  VR_VOLTAGE_350_MV << 6;
	} else if (CONFIG(tle8888VrDetectionVoltage) == V_550_MV) {
		vrsconfig0 |=  VR_VOLTAGE_550_MV << 6;
	}

	tle8888_spi_rw(chip, CMD_VRSCONFIG0(vrsconfig0), NULL);
		return 0;
}
